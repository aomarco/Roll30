...
The range indicator is drawn as nested translucent **diamond bands** centered on
the attacker (a single lightweight SVG, not per-cell squares — see the
range-indicator "To Be Updated" entry). Each band's color marks a range tier and
its outer edge is the maximum valid range for that tier.
...
- **Range-indicator perf — vector diamonds (2026-07-25):** The attack-range highlight used to render one animated DOM `<i>` per in-range cell, which for ranged weapons meant hundreds–thousands of nodes and heavy lag even at normal zoom. Replaced it entirely with a single `.range-layer` SVG that draws the range as a few nested **diamond polygons** (Manhattan-distance bands: green = normal, yellow = long, plus red for thrown long) centered on the attacker — a constant handful of DOM nodes regardless of weapon range or zoom. The SVG is a zero-size anchor at world (0,0) with polygon points in world pixels, so it scales with the world transform; translucent fills let the grid show through, with a light `range-pop` scale-in (disabled under reduced motion). `src/main.jsx` `attackMode` block + `.range-layer`/`.range-band` CSS in `src/movement.css`; `weaponRangeCells` is no longer used by the UI. `npm test` (86) / `npm run build` pass; render-verified.
