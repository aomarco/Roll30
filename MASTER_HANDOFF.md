...
range-indicator "To Be Updated" entry): translucent **diamond bands** centered on
the attacker, with real grid lines drawn through them (an SVG grid `pattern`
clipped to the range diamond, aligned to the board's cells) so the region reads
as squares without rendering hundreds of them. Each band's color marks a range
tier and its outlined edge is the maximum valid range for that tier.
