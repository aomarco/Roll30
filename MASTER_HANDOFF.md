...
- **Range-indicator perf (2026-07-25):** The attack-range highlight previously generated a weapon's full range diamond and filtered it against the *world* dimensions (`rect.width / zoom`), which explode when zoomed out — long-range ranged weapons spawned thousands of animated cells and lagged badly. Rewrote the `attackMode` cell block in `src/main.jsx` to iterate only the cells inside the on-screen **viewport** (screen rect mapped into world-local cell indices via the camera), coloring each with `weaponRangeBand` directly and capping at 1800 cells. This is bounded regardless of range or zoom (melee is a handful of cells; ranged fills only what's visible), eliminating the lag while keeping the animated look. `npm test` (86) / `npm run build` pass.
