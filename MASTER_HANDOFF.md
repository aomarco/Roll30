...
- **Bonus option button full width (2026-07-24):** The Bonus Actions option button was still collapsing to a bare icon square. Rather than rely on grid item auto-stretch, `.bonus-options` and `.bonus-options > button` now set explicit `width: 100%` in `src/premium.css`, guaranteeing the button fills the panel and shows its icon, title, and subtitle. `npm run build` passes.
