...
- Dropping a token on an out-of-range (red) square does not cancel the move: it travels as far along the traced path as movement allows, stopping on the last reachable (green) square and spending that movement. If the landing square is occupied it steps back to the nearest free square behind it. `farthestReachableIndex` (`src/combatRules.js`) computes the stop index.
...
the board's cells) so the region reads as actual squares without rendering
hundreds of them. Each band's color marks a range tier and its outlined edge is
the maximum valid range for that tier.
...
**House-ruled ranges:** all ranged weapons are deliberately nerfed from the SRD to a battle-map scale (Roll30 is a fun game, not a realistic simulator). Normal/long ranges are: Blowgun 25/50, Sling 30/60, Hand crossbow 40/80, Shortbow 40/80, Light crossbow 60/120, Heavy crossbow 80/160, Longbow 80/160 ft. The weapon source-fidelity test skips the range assertion for `rangeType: "ranged"` weapons since the deviation is intentional (thrown-weapon ranges are unchanged from the SRD).
...
- **Lock derived stats in setup for character tokens (2026-07-26):** In battle setup the `SETUP STATS` editor (HP, Max HP, Speed, Strength, Dexterity, Level, Initiative bonus, Creature size) let you overwrite values on **any** token, including ones spawned from a character sheet (which have `characterId`) whose stats are derived. Now those fields render as **read-only tiles** (`.setup-stat-readonly` in `src/studio.css`) for character-derived tokens, with the heading note "Derived from the character sheet — edit them there."; blank/manual tokens (no `characterId`) keep the editable number inputs and size select exactly as before. Only the stat grid is gated — the pre-equipped loadout and armour editors are still available per-battle. (Reminder for users: worn magic items and `+X` enchantments are also configured on the character sheet, not in the setup token panel — the **Wear** toggle and enchant stepper live on the inventory rows there.) `npm run build` passes; render-verified the read-only tiles.
- **Add Item picker: clipping fix + richer filters (2026-07-26):** Fixed a layout bug where the `.item-picker` declared `grid-template-rows: auto auto minmax(0,1fr)` for its **four** children (title, search+type, filter row, results), so the filter-row selects overlapped the results list. Now `auto auto auto minmax(0,1fr)` (dialog also widened to 460px / taller to 82vh). `.item-filter-row` changed from a fixed 2-col grid to a wrapping flex (`flex: 1 1 130px` per select) so a variable number of filters flow cleanly. Filters are now **context-aware**: the class/rarity select only shows when it has options (with type-appropriate "All …" label), and weapon-only filters appear solely for Weapons/All. Added three new filters in `src/items.js` `filterCatalog` (`{ damageType, range, sort }`) plus exports `DAMAGE_TYPES`, `RANGE_BANDS` (Melee/Ranged/Thrown, derived from weapon `rangeType` + Thrown property), and `SORT_OPTIONS` (Name A–Z default, Cost low→high / high→low via a copper-piece conversion). Results now sort by **name by default** (previously catalog order) across every `filterCatalog` caller (character picker, token quick-inventory, chest fill). `src/CharactersPage.jsx` gained `itemDamageType`/`itemRange`/`itemSort` state (reset on type change) and the rebuilt filter row; removed the now-unused `propertyEnabled`. Render-verified the fixed, unclipped picker with all five filters. `npm test` (106) / `npm run build` pass.
