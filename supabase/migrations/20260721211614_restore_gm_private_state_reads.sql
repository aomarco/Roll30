drop policy if exists "members read revealed scene objects" on public.scene_objects;
...
drop policy if exists "players read authorized map tiles" on public.scene_map_tiles;
...
drop policy if exists "players read own session token" on public.session_tokens;
