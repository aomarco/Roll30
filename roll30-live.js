(function () {
  ...
  async function content(view) {
    ...
    if (view === 'session') {
      ...
      const visibleFromLight = target => sceneObjects.filter(o=>o.object_type === 'light' && o.state?.active !== false).some(light => Math.hypot(target.x-light.x,target.y-light.y) <= (Number(light.config?.radius) || 20) && !blocked(light,target) && (!playerToken || (Math.hypot(playerToken.x-light.x,playerToken.y-light.y) <= vision && !blocked(playerToken,light))));
      const visibleTokens = !playerToken || currentRole === 'gm' || state.fog !== true ? (state.tokens || []) : (state.tokens || []).filter(t => t.id === playerToken.id || (Math.hypot(t.x-playerToken.x,t.y-playerToken.y) <= vision && !blocked(playerToken,t)) || visibleFromLight(t));
