(function () {
  ...
  function bind(view) {
    ...
    if (initiativeForm) initiativeForm.onsubmit = async e => { e.preventDefault(); const id = localStorage.getItem('roll30.sessionId'); const { data: current, error:getError } = await db.client.from('sessions').select('*').eq('id',id).single(); if(getError) return notice(getError.message,true); const token = (current.state?.tokens || []).find(t=>t.id === document.getElementById('initiative-token').value); if(!token) return; const entries = [...(current.state?.initiative || []),{token_id:token.id,name:token.name,score:Number(document.getElementById('initiative-score').value) || 0}].sort((a,b)=>b.score-a.score); const { error } = await db.client.from('sessions').update({state:{...current.state,initiative:entries},active_turn:0}).eq('id',id); if(error) notice(error.message,true); else render('session'); };
    app.querySelectorAll('[data-remove-initiative]').forEach(b => b.onclick = async () => { const id=localStorage.getItem('roll30.sessionId'); const { data: current, error:getError }=await db.client.from('sessions').select('*').eq('id',id).single(); if(getError)return notice(getError.message,true); const entries=(current.state?.initiative || []).filter((_,index)=>index!==Number(b.dataset.removeInitiative)); const { error }=await db.client.from('sessions').update({state:{...current.state,initiative:entries},active_turn:0}).eq('id',id); if(error)notice(error.message,true);else render('session'); });
  ...
  function openObjectDialog(sceneId) {
    const dialog = document.getElementById('live-dialog');
    dialog.innerHTML = `<form method="dialog" id="object-form"><h3>Interactive object</h3><input id="object-name" placeholder="Name" required><select id="object-type"><option value="object">Object</option><option value="door">Door</option><option value="lever">Lever</option><option value="trap">Trap</option><option value="light">Light</option><option value="wall">Wall</option></select><label>X position<input id="object-x" type="number" min="0" max="100" value="50"></label><label>Y position<input id="object-y" type="number" min="0" max="100" value="50"></label><label>Wall end X<input id="object-x2" type="number" min="0" max="100" value="60"></label><label>Wall end Y<input id="object-y2" type="number" min="0" max="100" value="50"></label><button>Add object</button></form>`;
    dialog.showModal();
    dialog.querySelector('#object-form').onsubmit = async e => { e.preventDefault(); const type=dialog.querySelector('#object-type').value; const { error } = await db.client.from('scene_objects').insert({scene_id:sceneId,name:dialog.querySelector('#object-name').value.trim(),object_type:type,x:Number(dialog.querySelector('#object-x').value),y:Number(dialog.querySelector('#object-y').value),config:type === 'wall' ? {x2:Number(dialog.querySelector('#object-x2').value),y2:Number(dialog.querySelector('#object-y2').value)} : {}}); if(error) return notice(error.message,true); dialog.close(); render('session'); };
  }
