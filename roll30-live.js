...
(function () {
  ...
  async function content(view) {
    ...
    if (view === 'builder') {
      ...
      const pieces=objects.filter(o=>o.object_type!=='wall').map(o=>`<button class="builder-piece" data-builder-object="${o.id}" style="left:${o.x}%;top:${o.y}%;z-index:${10+o.z_index}" title="Drag ${esc(o.name)}">${esc(o.name.slice(0,2).toUpperCase())}</button>`).join('');
      const walls=objects.filter(o=>o.object_type==='wall'&&o.config?.x2!=null).map(o=>{const dx=o.config.x2-o.x,dy=o.config.y2-o.y;return `<span class="map-wall" style="left:${o.x}%;top:${o.y}%;width:${Math.hypot(dx,dy)}%;transform:rotate(${Math.atan2(dy,dx)*180/Math.PI}deg)"></span>`;}).join('');
      return `<section><div class="live-title"><div><h2>${esc(scene.name)} builder</h2><p class="muted">Drag objects to snap them to the scene grid. Changes save immediately.</p></div><div class="session-controls"><button id="builder-back">Back to scenes</button><button id="builder-add">Add object</button></div></div><div class="session-board builder-board" id="builder-board">${pieces}${walls}${objects.length?'':'<span class="board-empty">Add an object, door, light, trap, lever, or wall.</span>'}</div><h3>Scene layers</h3>${cards(objects,o=>`<b>${esc(o.name)}</b><small>${esc(o.object_type)} · ${esc(o.layer)} layer · ${o.visible_to_players?'visible to players':'GM only'}</small><div class="hp-actions"><button data-builder-reveal="${o.id}" data-visible="${o.visible_to_players}">${o.visible_to_players?'Hide':'Reveal'}</button><button data-builder-layer="${o.id}" data-layer="${o.layer}" data-x="${o.x}" data-y="${o.y}">Change layer</button><button data-builder-delete="${o.id}">Delete</button></div>`)}</section>`;
    }
    if (view === 'characters') {
      const { data = [] } = await query('characters');
      const canEdit = c => currentRole === 'gm' || c.owner_id === session.user.id;
      return `<section><div class="live-title"><div><h2>Characters</h2><p class="muted">Versioned 5e sheets, ownership, resources, and campaign-ready NPCs.</p></div><button data-dialog="character">New character</button></div>${cards(data, c => { const abilities=c.sheet?.abilities || {}; const attack=c.sheet?.attacks?.[0] || c.sheet?.attack; const resources=(c.sheet?.resources || []).filter(resource=>resource?.name); return `<b>${esc(c.name)}</b><small>${esc(c.kind)} · ${c.hp_current ?? '—'} / ${c.hp_max ?? '—'} HP · AC ${esc(c.sheet?.armor_class ?? '—')}${c.sheet?.class ? ` · ${esc(c.sheet.class)} ${esc(c.sheet.level || '')}` : ''}${c.sheet?.conditions?.length ? ` · ${esc(c.sheet.conditions.join(', '))}` : ''}</small>${Object.keys(abilities).length ? `<div class="ability-strip">${['str','dex','con','int','wis','cha'].map(key=>`<span>${key.toUpperCase()} <b>${abilityModifier(abilities[key]) >= 0 ? '+' : ''}${abilityModifier(abilities[key])}</b></span>`).join('')}</div>` : ''}${resources.length ? `<div class="resource-list">${resources.map(resource=>`<button data-use-resource="${esc(resource.name)}" data-character="${c.id}" ${canEdit(c) && resource.current>0 ? '' : 'disabled'}>${esc(resource.name)} ${esc(resource.current)}/${esc(resource.max)}</button>`).join('')}</div>` : ''}<div class="hp-actions"><button data-hp="-1" data-character="${c.id}">− HP</button><button data-hp="1" data-character="${c.id}">+ HP</button>${canEdit(c) ? `<button data-edit-sheet="${c.id}">Edit sheet</button><button data-rest="short" data-character="${c.id}">Short rest</button><button data-rest="long" data-character="${c.id}">Long rest</button><button data-export-character="${c.id}">Export</button><button data-delete-character="${c.id}" data-character-name="${esc(c.name)}">Delete</button>` : ''}${canEdit(c) && attack?.name ? `<button data-attack="${c.id}">Attack</button>` : ''}</div>`; })}</section>`;
    }
    if (view === 'messages') {
    ...
    if (view === 'items') {
      ...
      return `<section><div class="live-title"><div><h2>Items</h2><p class="muted">Campaign equipment, consumables, spell records, and grantable loot.</p></div><button data-dialog="item">New item</button></div>${cards(data, i => `<b>${esc(i.name)}</b><small>${esc(i.item_data?.type || 'Custom item')}${i.item_data?.effect?.healing ? ` · heals ${esc(i.item_data.effect.healing)} HP` : ''}</small><form class="grant-item" data-item="${i.id}"><select aria-label="Character receiving item">${characters.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}</select><input type="number" min="1" value="1" aria-label="Quantity"><button>Grant</button></form>`)}</section>`;
    }
    if (view === 'inventory') {
      ...
      return `<section><div class="live-title"><div><h2>Character inventory</h2><p class="muted">Equip gear, consume supplies, or transfer an item to another campaign character.</p></div></div>${cards(data, i => { const allowed=currentRole === 'gm' || i.characters?.owner_id === session.user.id; return `<b>${esc(i.items?.name || 'Item')}</b><small>${esc(i.characters?.name || 'Character')} · quantity ${esc(i.quantity)}${i.metadata?.equipped ? ' · equipped' : ''}</small>${allowed ? `<div class="hp-actions"><button data-inventory-operation="${i.metadata?.equipped ? 'unequip' : 'equip'}" data-character="${i.character_id}" data-item="${i.item_id}">${i.metadata?.equipped ? 'Unequip' : 'Equip'}</button><button data-inventory-operation="consume" data-character="${i.character_id}" data-item="${i.item_id}">Consume one</button></div><form class="transfer-item" data-character="${i.character_id}" data-item="${i.item_id}"><select aria-label="Transfer destination">${characters.filter(c=>c.id !== i.character_id).map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}</select><input type="number" min="1" max="${i.quantity}" value="1" aria-label="Transfer quantity"><button>Transfer</button></form>` : ''}`; })}</section>`;
    }
    if (view === 'notes') {
      const isGm = currentRole === 'gm';
  ...
  function bind(view) {
    ...
    app.querySelectorAll('[data-builder-object]').forEach(piece=>{piece.onpointerdown=event=>{event.preventDefault();piece.setPointerCapture(event.pointerId);const move=moveEvent=>{const rect=builderBoard.getBoundingClientRect(),snap=5,x=Math.round(Math.max(0,Math.min(100,(moveEvent.clientX-rect.left)/rect.width*100))/snap)*snap,y=Math.round(Math.max(0,Math.min(100,(moveEvent.clientY-rect.top)/rect.height*100))/snap)*snap;piece.style.left=`${x}%`;piece.style.top=`${y}%`;piece.dataset.x=x;piece.dataset.y=y;};piece.onpointermove=move;piece.onpointerup=async()=>{piece.onpointermove=null;const{error}=await db.client.rpc('move_roll30_scene_object',{target_object:piece.dataset.builderObject,target_x:Number(piece.dataset.x||parseInt(piece.style.left)),target_y:Number(piece.dataset.y||parseInt(piece.style.top)),target_layer:null,target_z:null});if(error)notice(error.message,true);else notice('Object position saved.');};};});
  ...
  async function openDialog(kind) {
    ...
    dialog.innerHTML = kind === 'scene'
      ? `<form method="dialog" id="create-record"><h3>New scene</h3><label>Scene name<input id="record-name" required></label><label>Scene type<select id="record-type"><option value="playing">Playing field</option><option value="battle">Battle field</option></select></label><label>Folder<input id="record-folder" placeholder="Optional, such as Chapter 1"></label><button>Create scene</button></form>`
        ...
        : kind === 'shop'
          ...
          : kind === 'item'
            ? `<form method="dialog" id="create-record"><h3>New item</h3><label>Name<input id="record-name" placeholder="Item name" required></label><label>Type<select id="record-body"><option value="equipment">Equipment</option><option value="consumable">Consumable</option><option value="spell">Spell</option><option value="quest item">Quest item</option></select></label><label>Healing when consumed<input id="item-healing" type="number" min="0" value="0"></label><button>Create item</button></form>`
            : kind === 'trigger'
    ...
    dialog.querySelector('#create-record').onsubmit = async e => {
      ...
      const row=kind === 'scene' ? {campaign_id:campaignId,name,scene_type:type,folder,created_by:session.user.id} : kind === 'prompt' ? {campaign_id:campaignId,created_by:session.user.id,title:name,body} : kind === 'shop' ? {campaign_id:campaignId,name} : kind === 'item' ? {campaign_id:campaignId,name,item_data:{type:body || 'equipment',effect:{healing:Number(dialog.querySelector('#item-healing')?.value) || 0}}} : kind === 'trigger' ? {scene_id:type,name,trigger:{description:body||'Manual trigger'},effects:[{type:effect}]} : {campaign_id:campaignId,name,kind:type,owner_id:type === 'pc' ? session.user.id : null,hp_current:imported?.hp_current ?? 10,hp_max:imported?.hp_max ?? 10,sheet_schema_version:1,sheet:imported?.sheet || {abilities:{str:10,dex:10,con:10,int:10,wis:10,cha:10},level:1,proficiency_bonus:2,armor_class:10,speed:30,vision:30,conditions:[],features:[],equipment:[],attacks:[],currency:{gp:0}}};
